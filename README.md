...
The PriceDataLoader utilizes `Task Parallel Library (TPL)` and streaming I/O to efficiently ingest large financial datasets from a JSON file (portfolio_prices.json from [kaggle](https://www.kaggle.com/datasets/nikitamanaenkov/stock-portfolio-data-with-prices-and-indices?resource=download)) . Instead of loading the entire file into memory at once, it processes records one-by-one using:
